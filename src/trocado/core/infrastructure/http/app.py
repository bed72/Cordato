...
def build() -> Litestar:
    ...
    through the layered scope (feature router → version router → app).
    ...
    **Auth infrastructure** (``person_repository``, ``session_repository``, ``validate_session``, and
    ``current_person_provider``) is registered at the ``/v1`` router level — not per-feature — because
    ``CurrentPersonProvider`` is a cross-cutting provider that any protected handler may trigger. Each
    ``build()`` call produces fresh singletons, keeping test runs isolated.

    Each feature router builds and owns its own repositories and adapters internally. Cross-feature adapters
    (``PersonDirectory``, ``PartnerExpenseReader``, ``PartnerBudgetReader``, ``SpendReader``) live in the
    consuming feature's ``gateways/`` package.
    """
    api = Router(
        path="/v1",
