...
because the money value object lives in the shared kernel. ``InvalidSessionError`` is core because the auth
provider (``CurrentPersonProvider``) is wired at the ``/v1`` router level and can raise it from any feature's
protected handler — it must be handled everywhere, not just in the identity router.
"""
