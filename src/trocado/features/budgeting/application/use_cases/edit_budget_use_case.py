...
class EditBudgetUseCase:
    """Edit one of the requester's own live budgets: resolve it scoped to the owner, apply the new values,
    ...
    async def execute(self, data: EditBudgetData) -> BudgetData:
        ...
        # Apply the edit to a throwaway copy: `edit` re-validates amount then range there, and the proposed
        # range is tested for overlap — all before anything is persisted, so any rejection changes nothing.
