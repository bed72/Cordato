...
def build_app() -> Application:
    """Assemble the BlackSheep application — the thin top-level composition root.
    ...
    Deliberately the **bare bootstrap**: it brings the happy path online and nothing more. Error→HTTP
    framing (the exception handlers / status table) and the transitional ``X-Person-Id`` identity are left
    to their own tasks of this change — not wired here.
    """
    container = Container()
