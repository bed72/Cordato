...
internal fun SignUpError.toResponse(messages: MessagePort): HttpResponse<ErrorResponse> = when (this) {
    ...
    SignUpError.InvalidEmail ->
        unprocessable("INVALID_EMAIL", messages("signup.error.invalidEmail"))
    SignUpError.EmailAlreadyInUse ->
        unprocessable("SIGNUP_REJECTED", messages("signup.error.emailAlreadyInUse"))
    is SignUpError.WeakPassword ->
        unprocessable("WEAK_PASSWORD", messages("signup.error.weakPassword", mapOf("minLength" to minLength)))
}
