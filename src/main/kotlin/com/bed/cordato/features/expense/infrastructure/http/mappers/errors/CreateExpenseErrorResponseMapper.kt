...
/**
 * Maps each domain [CreateExpenseError] to an HTTP status and a neutral [ErrorResponse], as an `internal`
 ...
 * never signals *which* rejection occurred, and none is emitted as a per-field `FieldErrorResponse` (that is
 * the edge `400` path only). The message is resolved by i18n key; the `code` stays an inline constant — the
 * machine contract, never localized.
 */
internal fun CreateExpenseError.toResponse(messages: MessagePort): HttpResponse<ErrorResponse> = when (this) {
