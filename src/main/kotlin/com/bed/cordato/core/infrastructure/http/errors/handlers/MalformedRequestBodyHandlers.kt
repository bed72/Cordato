 ...
 * there is no per-field breakdown — [ErrorResponse.errors] stays empty and the failure is scalar, in the
 * same shared shape as every other rejection. "Can't be read" arrives as three different framework
 ...
 * `_embedded.errors` body) with the same generic, non-leaking `400`.
 */
...
class JsonSyntaxExceptionHandler :
    ...
    override fun handle(request: HttpRequest<*>, exception: JsonSyntaxException): HttpResponse<ErrorResponse> =
        badRequest("MALFORMED_REQUEST", "O corpo da requisição está ausente ou é inválido.")
}
 ...
 * scalar rather than filling [ErrorResponse.errors].
 */
@Produces
@Singleton
