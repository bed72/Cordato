 ...
 *
 * The body is fixed and neutral: a stable [code] and a generic [message], with no [ErrorResponse.errors].
 * The exception itself — message, stacktrace, any SQL/path/type detail — is written **only** to the server
 * log, never serialized, honouring the system's non-leak invariant (an error response must not become an
 * oracle). The operator gets the detail; the client gets nothing exploitable.
 */
@Produces
@Singleton
class UnexpectedFailureExceptionHandler :
    ...
    override fun handle(request: HttpRequest<*>, exception: Throwable): HttpResponse<ErrorResponse> {
        ...
        return internalError("INTERNAL_ERROR", "Ocorreu um erro inesperado. Tente novamente mais tarde.")
    }
}
