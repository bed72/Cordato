 ...
 *
 * The body is fixed and neutral: a stable code and a generic message resolved by key from the
 * request-scoped [LocalizedMessageSource], with no [ErrorResponse.errors]. The exception itself — message,
...
@Produces
@Singleton
class UnexpectedFailureExceptionHandler(private val messages: LocalizedMessageSource) :
    ...
    override fun handle(request: HttpRequest<*>, exception: Throwable): HttpResponse<ErrorResponse> {
        ...
        return internalError("INTERNAL_ERROR", messages.resolve("error.internal.message"))
    }
}
