...
A slice HTTP do contexto `expense`: expõe `POST /expenses` como rota protegida (exige sessão viva),
recebe o registro de um gasto como JSON, valida a forma no edge (Bean Validation), delega ao caso de uso
e traduz o resultado selado para uma resposta HTTP. Reusa o contrato de erro cross-cutting do `core`
(`400`/`422`/`500`), resolve todo texto por chave i18n e documenta a rota via OpenAPI em compile-time.
