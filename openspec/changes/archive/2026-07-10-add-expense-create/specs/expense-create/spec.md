...
Registro de um novo gasto (o **fato atômico** do domínio) para o ator autenticado no contexto `expense`.
Cobre a validação do valor (exato, em centavos, sempre > 0), da data (opcional, default hoje, futuro
recusado) e da descrição (opcional, aparada, limitada); a criação de um `Expense` pertencente a
exatamente uma pessoa; e o princípio de que o gasto nunca referencia orçamento, registrando apenas o fato
bruto (quem, quanto, quando, descrição).
