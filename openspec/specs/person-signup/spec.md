...
Cadastro de uma nova pessoa no contexto `identity` — a âncora de todo o sistema. Cobre a validação de
e-mail, nome e política de senha; a unicidade de e-mail (verificada antes do hashing); a criação de uma
`Person` ativa com a senha armazenada apenas como hash irreversível; e a redação de erros que não vaza a
existência de conta.
