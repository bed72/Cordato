# application-layer-structure Specification

## Purpose

A convenção estrutural da camada `application/` de cada módulo: como suas subpastas de categoria são
agrupadas fisicamente. O agrupamento é **pela direção da porta** do hexágono — `driving/` (portas
primárias/inbound) e `driven/` (portas secundárias/outbound) — com `mappers/` neutro na raiz. É uma capability
estrutural (verificável por inspeção do código-fonte, sem comportamento de runtime), no mesmo espírito de
`api-versioning` e `dependency-injection`, que codifica a distinção *driving*/*driven* que a arquitetura já
assume e proíbe baldes genéricos de agrupamento.
