# http-integration-tests Specification

## Purpose

Define the convention for HTTP integration tests that drive the real Litestar application via
`TestClient`, verifying routing, the unified error envelope, and in-memory store state — without
mocking the web framework or the use cases.
...
SHALL viver em `tests/<ctx>/integrations/http/` e seguir a convenção de um arquivo por recurso HTTP testado,
nomeado `test_<resource>_http.py`.
