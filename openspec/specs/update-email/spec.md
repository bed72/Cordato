# update-email Specification

## Purpose
Lets an authenticated person change their own email (the login identifier) in place: the current password
re-confirms identity (no oracle), the new email is validated, normalized, and re-checked for uniqueness among
other active people (no enumeration), and every other session is purged while the acting one survives —
credential, identity, and ledger untouched.
## Requirements
