# couple-budget Specification

## Purpose
The couple is a point of view, not an owner — this capability is the read-only couple-level budget view, the
combined panorama over both partners' active budgets. For a reader in a live pair it returns a single view
spanning `[min(start), max(end)]` of the present active budgets, with `amount`, `total_spent`, and
`remaining` summed across them. It is a deliberately approximate lens, not a merge: nothing is stored,
copied, or owned by the pair, the figures are derived at read-time, and the exact truth stays in each
person's own active budget. The panorama exists whenever at least one partner has an active budget; with
neither present there is no view, and a reader in no live pair is rejected. A missing active budget is never
fabricated into a "No budget" bucket. Partner active budgets are read through a port the `pairing` context
owns, never a cross-module dependency. The default-budget bucket and pair dissolution are separate
capabilities.
