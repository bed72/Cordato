# authentication-http-edge Specification

## Purpose

Defines the HTTP edge for the authentication surface: the sign-up, sign-in, and sign-out endpoints, the injectable `current_person` dependency that protects routes, and the `SessionResponse` shape returned on successful auth.
