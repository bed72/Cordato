# http-rate-limiting Specification

## Purpose

Um limite de requisições transversal, aplicado a **toda** rota HTTP via um `@ServerFilter` no `core`,
protegendo a borda de abuso (força bruta contra `sign-up`/`sign-in`, ou qualquer rota hostilizada por um
único cliente) sem introduzir nenhuma dependência nova — reaproveita o contador atômico já provido pela
capability `cache-valkey`. Roda antes do guard de autenticação, de modo que uma enxurrada de tokens
inválidos contra uma rota protegida é contada antes de disparar a consulta de sessão. A recusa por limite
excedido usa o mesmo envelope de erro compartilhado da capability `http-error-handling`.
