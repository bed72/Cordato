...
O log cross-cutting de request/response HTTP: um filtro de servidor que registra method/path/status/duração
de toda requisição via `LoggerPort` (ver [[structured-logging]]) e mina um correlation id por requisição,
propagado por MDC e devolvido ao cliente — a base sobre a qual um futuro trace id de OpenTelemetry vai
pousar sem exigir mudança de call site.
