...
O kernel de **logging estruturado** no `core`: um `LoggerPort` único, consumido por `application/` e
`infrastructure/`, com atributos restritos a um tipo fechado (`LoggableValueObject`) e uma segunda camada
de proteção — uma denylist de nomes de atributo sensíveis — no adapter que o implementa sobre SLF4J/Logback.
Desenhado para que uma futura integração com OpenTelemetry Logs seja uma troca de adapter, não uma
reescrita de call site.
